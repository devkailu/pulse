...
export default function SignupUser() {
  ...
  const onChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setForm((prev) => ({
      ...prev,
      [name]: name === "subscription_id" ? Number(value) : value,
    }));
  };
  ...
  const onFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      console.log("[SignupUser] selected avatar:", file);
    }
  };
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("[SignupUser] signup response:", res);

      // Save user including avatar_url into Zustand & localStorage
      setUser(res.user);
      console.log("[SignupUser] user saved in store:", res.user);
    ...
    >
      ...
      >
        ...
        >
          ...
          <option value={2}>Premium (demo)</option>
        </select>
        ...
        <button
          type="submit"
          className="w-full py-3 rounded bg-indigo-500 hover:bg-indigo-400 font-semibold"
        >
          Create Account
        </button>
      </form>
